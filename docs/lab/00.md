...
| `alloc` | Provides *heap* allocated data structures like, `Box` and `Vec`. The developer has to provide a memory allocated, like [embedded_alloc](https://docs.rs/embedded-alloc/latest/embedded_alloc/). | Memory Allocator |
| `std` | Provides all the functionality of Rust, including threads and files I/O. This is the default level for Windows, Linux, macOS and similar OSes applications. | Operating System |
