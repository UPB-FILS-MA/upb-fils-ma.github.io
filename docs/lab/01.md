...
:::danger
If there is no resistor in the circuit, the resistance will be almost 0 &ohm;, the current will tend to `infinity`, meaning a short circuit. This will absolutely burn the LED and make it unusable, but it can also burn the MCU. Most MCUs have short circuit protection, but is safer to not rely on that.  
:::
...
:::info
For a better understanding, please read the chapters 5,10,26 and 28 of *Encyclopedia of Electronic Components*, Volumes 1
:::
...
- A status LED is ON as long as the microcontroller is supplied. The pin that determines the status of the LED is TBD.
- A button is used to toggle the LED integrated on the development board. The pin reading the state of the button is TBD.
- On the screen is displayed an analog value read by the microcontroller on pin TBD. Create a circuit in order to display the value TBD(can be customized) on the screen.

:::warning
Do not supply the microcontroller before the lab teacher checks the circuit.
:::
