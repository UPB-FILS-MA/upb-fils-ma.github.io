...
Electronic voltage represents the potential difference between 2 points in a circuit and it's proportional to the energy required to move an electric charge between the 2 points.
...
Electronic voltage is always measured between 2 pints in a circuit. In general, voltages are measured against a reference point called circuit mass **(GND)**. Circuit mass is a convention and represents the point which potential is consider null. 
...
:::info
In case we want to use the voltage divider between 2 chosen voltage values, we can use the generalized formula:
