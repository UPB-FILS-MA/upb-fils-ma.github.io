...
If the MCU provides timers, Embassy-rs will use them to suspend the software. This is very efficient.
...
Embassy-rs provides the `embassy_executor::main` macro that
sets the `main` function as the startup point, by also using the `entry` macro, and defines the correct bootloader.
...
2. Write a Rust program that set on HIGH the LED connected to GPIO pin 0 (GP0). (**2p**)
   1. use the `embassy-rs` framework
...
3. Write a Rust program that blinks the LED connected to GPIO pin 0 (GP0) every 300ms. (**3p**)
   ...
   3. use bare metal - write a `PinDriver` that implements the [`OutputPin`](https://docs.rs/embedded-hal/latest/embedded_hal/digital/trait.OutputPin.html) trait
