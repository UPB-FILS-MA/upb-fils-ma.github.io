...
a  set of standard traits (interfaces) for interacting with an MCU. This is called the **Embedded Hardware Abstraction Layer**, or shortly Embedded HAL. The mai purpose is to define a common hardware interface that
...
The `embassy_rp::init` function takes care of the peripheral initialization so that developers can jumo
right in and use them.

:::note

Embassy-rs is designed to work in an asynchronous way and this is why the `main` function is defined as `async`. For te time being, just take it as a must and ignore it.
...
the [`Timer`](https://docs.rs/embassy-time/0.3.0/embassy_time/struct.Timer.html) structure form the
...
1. Use [KiCad](https://www.kicad.org/) to design a simple circuit that connects an LED to GPIO 0 (GP0).
...
2. Write a Rust program that set on HIGH the LED connected to GPIO pin 0 (GP0).
   1. use bare metal
...
3. Write a Rust program that blinks the LED connected to GPIO pin 0 (GP0) every 300ms.
   1. use bare metal
   2. use the `rp2040-pac` crate
   3. use the `embassy-rs` framework
...
4. Write a program using `embassy-rs` that will write the message "The button was pressed" to the console every time button A is pressed. Take a look at the Pico Explorer Base's pinout to determine the pin to which button A is connected.
...
5. Write a Rust program using `embassy-rs` that toggles the LED every time button A is pressed.
